...
Keep it concise, sharp, and actionable."""
            ...
            with st.spinner("Claude is analyzing your data…"):
                try:
                    ...
                    st.markdown(f'<div class="insight-card"><p>{answer}</p></div>',
                                unsafe_allow_html=True)
                    buf = io.StringIO()
                    buf.write(f"DataLens AI\nQuestion: {question}\n\n{answer}")
                    st.download_button("⬇️ Download analysis", buf.getvalue(),
                                       file_name="datalens_ai_analysis.txt", mime="text/plain")
